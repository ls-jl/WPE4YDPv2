...
void MediaPlayerPrivateGStreamer::configureElement(GstElement* element)
{
    ...
    // Rockchip 视频 overlay 直出：仅当本管线挂的是 hole-punch sink（sink 上带
    // 旋转标记）时，让 mppvideodec 用 RGA 按面板方向预旋转输出；软件回退管线
    // 不受影响。
    if (m_videoSink) {
        if (const char* holePunchRotation = static_cast<const char*>(g_object_get_data(G_OBJECT(m_videoSink.get()), "wpe-holepunch-rotation"))) {
            auto* factory = gst_element_get_factory(element);
            if (factory && !g_strcmp0(GST_OBJECT_NAME(factory), "mppvideodec") && gstObjectHasProperty(element, "rotation"_s))
                gst_util_set_object_arg(G_OBJECT(element), "rotation", holePunchRotation);
        }
    }
