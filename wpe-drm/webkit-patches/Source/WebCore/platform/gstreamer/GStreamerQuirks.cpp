...
#undef ADD_QUIRK
    ...
    if (WTF::equalLettersIgnoringASCIICase(identifierString.span(), "help"_s)) {
        gst_printerrln("Supported quirks for WEBKIT_GST_HOLE_PUNCH_QUIRK are: fake, bcmnexus, rialto, rockchip, westeros");
        return;
    }
    ...
    else if (WTF::equalLettersIgnoringASCIICase(identifierString.span(), "rockchip"_s))
        m_holePunchQuirk = WTF::makeUnique<GStreamerHolePunchQuirkRockchip>();
